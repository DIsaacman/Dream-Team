...
def convert_dollars(intent_request):
    ...
    currency_buy = get_slots(intent_request)["currency_buy"]
    currency_sell = get_slots(intent_request)["currency_sell"]
    ...
    # Get the current price of {currency_slot} in dolars and make the conversion from dollars.
    api_url = f"https://cdn.jsdelivr.net/gh/fawazahmed0/currency-api@1/latest/currencies/{currency_sell}/{currency_buy}.json"
    response = requests.get(api_url)
    response_json = response.json()
    exchange_rate = round(parse_float(response_json[currency_buy]),4)
    today = response_json['date']
    
    currency_buy = currency_buy.upper()
    currency_sell = currency_sell.upper()
    
    value = parse_float(dollars) * exchange_rate
    value = round(value, 4)
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            "content": """Thank you. As of {}, the exchange rate for {} / {} is {}
            you can get {} $ {}  for your {} {} dollars.
            . Please type "Borrow" to proceed if you want to borrow.
            """.format(today,currency_sell, currency_buy, exchange_rate,currency_buy,
                value, currency_sell, dollars
            ),
        },
    )
